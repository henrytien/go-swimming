...
func TestIntergration(t *testing.T) {
	...
	v := <-sleep // Wait for poll loop to start sleeping.
	t.Log(v, s)
	...
	<-done  // Wait for poller to see the "OK" status and eixt.

	// Mkae second request to the server.
	w = httptest.NewRecorder()
	s.ServeHTTP(w, r)
	if b := w.Body.String(); !strings.Contains(b, "YES!") {
		t.Fatalf("body = %s, want yes", b)
	}
